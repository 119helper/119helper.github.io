...
- 저장 공간 부족 시 상단에 실패 상태를 표시하며, 현재 탭에 남은 입력을 다시 저장할 수 있습니다.
- 저장 실패 안내가 남아 있으면 탭을 닫거나 새로고침하기 전에 재시도하세요.

### 💾 업무자료 백업·복원
- 설정 → 일반 → 업무자료 백업·복원에서 메모·일정·대상물과 사진·종료된 출동을 JSON으로 보관합니다 (최대 25MB).
- 복원 전 파일 형식·버전·사진을 검증하고 건수를 미리 보여줍니다. 기존 자료는 유지하고 없는 항목만 추가합니다.
- 진행 중인 출동과 앱 잠금 설정은 백업하지 않으며, 종료 출동에는 현재 기기의 보관 기간을 적용합니다.
- 저장 공간 부족으로 일부만 반영되면 결과를 안내합니다. 다시 복원해도 이미 추가한 항목은 중복되지 않습니다.
- 대상물 화면의 가져오기는 동일 ID에 대해 기존 유지·더 최신 내용 적용·별도 복사를 선택할 수 있습니다.
- 백업 파일은 암호화되지 않으므로 연락처·사진·환자 기록을 포함한 파일을 안전하게 보관하세요.

연결 상태의 ‘온라인’은 모든 정보가 최신임을 보장하지 않습니다. 최근 알림의 정보별 조회 상태에서 날씨·응급실·도로의 실패 여부와 마지막 성공 시각을 확인하세요.
