...
- 공중화장실 정적 데이터 갱신 workflow를 추가했다.
  ...
  - LOCALDATA 전국 CSV와 서울·제주시·부산 동래구 공식 CSV, 부산 갈맷길 공식 SHP를
    키 없이 내려받는다.
  - 지역 좌표는 이름+도로명/지번주소의 고유 1:1 일치와 관할 좌표 경계를 모두 통과한
    중앙 관리번호에만 보충하며 출처별 수량을 별도 인덱스에 남긴다.
...
`updtymd`를 사용한다. 2026-07-29 원본 갱신일을 확인했지만 DSSP-IF-10944 전수 조회는
강원·경북·부산·울산 647건이고, 행정안전부 점검 발표 관리대장은 680개소다.
