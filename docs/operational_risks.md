...
2026-08-01 국민안전24의 시군구별 정적 JSON과 안전지도 조회를 별도로 전수 확인한 결과,
두 목록은 각각 636곳이며 비교 키 기준 636곳 전부 서로 일치했다. 공개 API 647곳보다
11곳 적고 발표 680곳보다 44곳 적으므로, 680곳 명부의 대체 원본이 아니라 독립적인
교차검증 출처로만 사용한다.

권장 조치:
- 33곳을 임의 생성하지 않고 화면과 manifest에서 차이를 계속 표시한다.
- 국민안전24 정적 목록과 안전지도를 매일 자동 교차검증하되, 공개 API 원본에 합치지 않는다.
...
- 그 전까지 앱은 관할 재난문자의 도로 통제 표현을 `문자 기반·재확인 필요` 후보로
  별도 표시한다. 좌표·통제 구간·해제 여부가 없는 후보를 5km 내 확정 통제로 승격하거나
  변화 감시 기준선에 넣지 않는다.
