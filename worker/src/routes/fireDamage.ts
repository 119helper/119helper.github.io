...
import { encodeServiceKey, fetchWithRetry, parsePublicDataJson, isRecord, asArray } from './publicData';
...
export async function handleFireDamage(
  ...
  const res = await fetchWithRetry(`${BASE}/getOcByregionFpcnd?serviceKey=${serviceKey}&${params}`, {
