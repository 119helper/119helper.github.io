...
export function classifyAviationSafety(windSpeed: number | string | null | undefined): AviationSafetyResult {
  ...
  if (normalized === null) {
    return {
      ...
      colorClass: 'text-amber-800 dark:text-amber-200 bg-amber-500/10 border-amber-500/30',
      icon: 'help',
    };
  }
  ...
  if (normalized >= 10) {
    return {
      ...
      colorClass: 'text-red-800 dark:text-red-200 bg-red-500/10 border-red-500/30',
      icon: 'block',
    };
  }
  ...
  return {
    ...
    colorClass: 'text-green-800 dark:text-green-200 bg-green-500/10 border-green-500/30',
    icon: 'check_circle',
  };
}
