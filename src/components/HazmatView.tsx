      ...
      <div style={{ width: 120, height: 120, borderRadius: '50%', background: `conic-gradient(${gradient})`, position: 'relative', flexShrink: 0 }}>
        <div style={{ position: 'absolute', inset: '28%', borderRadius: '50%', backgroundColor: 'var(--color-surface-container-lowest, #060a14)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="text-center">
            ...
            <p className="text-xs text-on-surface-variant">총계</p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        <SummaryCard icon="factory" iconColor="text-red-700 dark:text-red-300" label="총 시설" value={selected.total} />
            ...
            {sortedDepts.map((d, i) => {
              ...
              return (
                <button
                  ...
                  aria-pressed={selectedDept === d.fireDept}
                  className={`w-full min-h-11 py-1.5 flex items-center gap-2 text-left rounded-lg transition-colors ${selectedDept === d.fireDept ? 'bg-primary/10' : 'hover:bg-surface-container/50'}`}
                >
                  <span className="text-xs text-on-surface-variant w-24 sm:w-28 text-right truncate px-1" title={d.fireDept}>{d.fireDept.replace('소방서', '')}</span>
                  <div className="flex-1 h-6 bg-surface-container rounded overflow-hidden relative">
                    ...
                    <span className="absolute right-2 top-1/2 -translate-y-1/2 text-xs font-bold text-on-surface">
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-surface-container border-b border-outline-variant/10">
                ...
                <th className="text-right px-3 py-2 font-bold text-red-700 dark:text-red-300">제조</th>
                <th className="text-right px-3 py-2 font-bold text-amber-700 dark:text-amber-300">주유</th>
                <th className="text-right px-3 py-2 font-bold text-green-700 dark:text-green-300">일반취급</th>
                <th className="text-right px-3 py-2 font-bold text-blue-700 dark:text-blue-300">옥외탱크</th>
                <th className="text-right px-3 py-2 font-bold text-purple-700 dark:text-purple-300">이동탱크</th>
                <th className="text-right px-3 py-2 font-bold text-cyan-700 dark:text-cyan-300">옥내</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-outline-variant/5">
              {HAZMAT_FACILITY_DATA.map(d => (
                <tr
                  key={d.fireDept}
                  tabIndex={0}
                  aria-selected={selectedDept === d.fireDept}
                  onClick={() => setSelectedDept(d.fireDept)}
                  onKeyDown={event => {
                    if (event.key === 'Enter' || event.key === ' ') {
                      event.preventDefault();
                      setSelectedDept(d.fireDept);
                    }
                  }}
                ...
                >
                  ...
                  <td className="px-3 py-2 text-right text-red-700 dark:text-red-300 tabular-nums">{d.manufacturing}</td>
                  <td className="px-3 py-2 text-right text-amber-700 dark:text-amber-300 tabular-nums">{d.handling.gasStation}</td>
                  <td className="px-3 py-2 text-right text-green-700 dark:text-green-300 tabular-nums">{d.handling.general}</td>
                  <td className="px-3 py-2 text-right text-blue-700 dark:text-blue-300 tabular-nums">{d.storage.outdoorTank}</td>
                  <td className="px-3 py-2 text-right text-purple-700 dark:text-purple-300 tabular-nums">{d.storage.mobile}</td>
                  <td className="px-3 py-2 text-right text-cyan-700 dark:text-cyan-300 tabular-nums">{d.storage.indoor}</td>
                </tr>
              ))}
            </tbody>
          </table>
